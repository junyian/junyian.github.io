...
Pretty straightforward so far. If we run the EXE, we get that prompt asking for
the password. By entering some random text we get the error message.

![c01](/assets/img/flareon/flareon2015/flareon2015c01_img01.PNG)
{: style="max-width: 80%" }
...
prompts. Eventually we'll get to a screen with a flowchart and ASM code.

![ida](/assets/img/flareon/flareon2015/flareon2015c01_img02.PNG)
...
Basically, this flow here can be summarised as:
(trying out mermaid-js, this graph might take a while to load)

```mermaid
graph TD
    Start[Prompt password]
    XOR(XOR each byte with `7Dh`, length `18h`)
    CMP(Compare result to hardcoded byte array at `byte_402140`)
    Success[Print success]
    Failure[Print failure]
    Start --> XOR
    XOR --> CMP
    CMP -->|Equal| Success
    CMP -->|Not equal| Failure
```

We can deduce that the flag should be related to the byte array located at
`byte_402140`, which is `18h` or 24 bytes long.

![byte_402140](/assets/img/flareon/flareon2015/flareon2015c01_img03.PNG)
...
1. Use the Windows calculator (I'm just trolling, but yeah, it is a valid option
...
print()
```
[View code in Github](https://github.com/junyian/flare-on-2015/tree/main/c01)
3. Use [Cyberchef][cyberchef]

![cyberchef](/assets/img/flareon/flareon2015/flareon2015c01_img04.PNG)
